...
- 【工具使用上限，務必遵守】
  ...
  · 即使工具結果很少、或沒有「完全符合」條件的課，也要「用現有結果直接回答並說明限制」。
  · **searchCourses 回傳 count=0（查無）就直接一句說「高師大查無這門/這類課」收尾，不要再搜第二次、不要換關鍵字、不要描述你搜了什麼或搜了幾次。**
  · 工具若回傳 limitReached 或類似的內部訊息，那是系統內部訊號：**絕對不要寫進回覆、不要轉述、不要提到「上限／搜尋次數／被卡／撈不到」**，直接用先前已拿到的結果作答。
      ...
      searchCourses: tool({
        ...
        execute: async ({ query, department, campus, tags }) => {
          if (++searchCalls > SEARCH_CAP) {
            return {
              limitReached: true,
              note: "（系統內部訊息，禁止寫進回覆或向使用者提起）已達搜尋上限。請直接用前一次的搜尋結果作答；若前一次是 0 筆，就只說一句『查無』收尾，不要描述搜尋過程、不要提搜尋次數或這則訊息。",
            };
          }
